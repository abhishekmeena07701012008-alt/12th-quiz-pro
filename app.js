...
// यह नया डीप-स्कैन फंक्शन है जो बिना क्रैश हुए हर डेटा ढूंढ लेगा
function startQuiz(subjectKey) {
    ...
    // रिकर्सिव सर्च फंक्शन जो एरे के अंदर के एरे को भी स्कैन कर लेगा
    function deepSearch(obj, inheritedSubject = '') {
        ...
        // सब्जेक्ट का पता लगाना
        let itemSub = obj.subject ? obj.subject.toLowerCase().replace(/_|\s/g, '') : inheritedSubject;
        if (itemSub.includes('math')) itemSub = 'math';

        let qText = obj.question || obj.q;

        // अगर सब्जेक्ट मैच कर गया और सवाल मौजूद है
        if (itemSub.includes(currentSubject) || currentSubject.includes(itemSub)) {
            if (qText && Array.isArray(obj.options)) {
                // भाषा चेक (अगर hi_en है या भाषा दी ही नहीं है, तो उसे पास कर दो)
                let langMatch = true;
                if (obj.lang) {
                    let l = obj.lang.toLowerCase();
                    if (l === 'hi' || l === 'en') {
                        if (l !== currentLanguage) langMatch = false;
                    }
                }
                ...
                if (langMatch) {
                    filteredQuestions.push({
                        question: qText,
                        options: obj.options,
                        correct: obj.correct !== undefined ? obj.correct : (obj.answer || 0)
                    });
                }
            }
        }

        // अगर इसके अंदर 'questions' या 'data' नाम का कोई और एरे है (जैसे आपके history.js के आखिर में है)
    ...
    // सर्च शुरू करें
    if (typeof quizDataMaster !== 'undefined') {
        deepSearch(quizDataMaster);
    }
