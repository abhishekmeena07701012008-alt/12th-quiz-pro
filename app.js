...
// स्ट्रीम डेटा मैपिंग (history.js की सभी विसंगतियों को संभालने के लिए सटीक कीज)
const streamData = {
    science: {
...
// 3. बुलेटप्रूफ क्विज़ स्टार्ट फ़ंक्शन (सभी 22 विषयों के मिक्स्ड डेटा फ़ॉर्मेट को ऑटो-हैंडल करेगा)
function startQuiz(subjectKey) {
    currentSubject = subjectKey;
    let filteredQuestions = [];

    if (typeof quizDataMaster !== 'undefined' && Array.isArray(quizDataMaster)) {
        quizDataMaster.forEach(item => {
            if (!item.subject) return;
            
            // विषय के नाम को पूरी तरह क्लीन करना ताकि स्पेलिंग (जैसे applied_math vs Applied Maths) मिसमैच न हो
            let dbSubject = item.subject.toLowerCase().replace(/_|\s/g, '');
            let appSubject = currentSubject.toLowerCase().replace(/_|\s/g, '');
            
            // 'maths' और 'math' दोनों को आपस में सही मैच कराने के लिए एक्स्ट्रा चेक
            if (appSubject === 'maths' || appSubject === 'math') {
                appSubject = 'math';
            }
            if (dbSubject.startsWith('math')) {
                dbSubject = 'math';
            }
            
            let isSubjectMatch = (dbSubject.includes(appSubject) || appSubject.includes(dbSubject));
            
            if (isSubjectMatch) {
                // फ़ॉर्मेट 1: अगर आइटम के अंदर एक और 'questions' नाम का सब-एरे है (जैसे Political Science में है)
                if (item.questions && Array.isArray(item.questions)) {
                    item.questions.forEach(subQ => {
                        filteredQuestions.push({
                            question: subQ.q || subQ.question,
                            options: subQ.options,
                            correct: subQ.correct !== undefined ? subQ.correct : (subQ.answer !== undefined ? subQ.answer : 0)
                        });
                    });
                } 
                // फ़ॉर्मेट 2: अगर ऑब्जेक्ट में सीधे प्रश्न और ऑप्शंस दिए हुए हैं
                else if (item.options) {
                    let itemLang = item.lang ? item.lang.toLowerCase() : '';
                    
                    // अगर डेटाबेस में lang नहीं लिखा है (जैसे Maths, CS में), या 'hi_en' लिखा है, तो इसे हर भाषा के लिए सही मानें
                    let langMatch = !item.lang || 
                                    itemLang === '' ||
                                    itemLang === currentLanguage.toLowerCase() || 
                                    itemLang.includes('hi') || 
                                    itemLang.includes('en');
                                    
                    if (langMatch) {
                        filteredQuestions.push({
                            question: item.question || item.q,
                            options: item.options,
                            correct: item.correct !== undefined ? item.correct : (item.answer !== undefined ? item.answer : 0)
                        });
                    }
                }
            }
    ...
    // अगर कोई सवाल नहीं मिले तो अलर्ट
